...
: ${DEFAULT_MSVCRT:=ucrt}
: ${MINGW_W64_VERSION:=5ea8e9facd013b815f5f29f20ef26a566319a3de}
...
for arch in $ARCHS; do
    ...
    mkdir -p build-$arch
    cd build-$arch
    ../configure --host=$arch-w64-mingw32 --prefix="$PREFIX/$arch-w64-mingw32" $FLAGS $CFGUARD_FLAGS $CRT_CONFIG_FLAGS CFLAGS="-g -Oz -fno-auto-import -ffunction-sections -fdata-sections"
    $MAKE -j$CORES
    $MAKE install
    cd ..
done
